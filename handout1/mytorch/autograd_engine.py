...
def backward(grad_fn, grad_of_outputs):
    ...
    for parent, grad_of_output in zip(grad_fn.next_functions, grad):
        if parent is None:
            continue
        grad_fn = parent.grad_fn
        backward(grad_fn, grad_of_output)
...
class Function:
    ...
    def apply(cls, *args):
        ...
        backward_function = BackwardFunction(cls)  # 反向传播的节点
        ...
        for arg in args:  # 对所有参加运算的参数，放到其next_functions中
             # 常数的情况，当进行reshape的时候不是tensor
            if not isinstance(arg, tensor.Tensor):
                backward_function.next_functions.append(None)
            elif arg.is_leaf == True and arg.requires_grad == True:
                # 对需要计算梯度的叶子节点特殊处理
                arg.grad_fn = AccumulateGrad(arg)
                backward_function.next_functions.append(arg)
            elif arg.is_leaf == True and arg.requires_grad == False:
                backward_function.next_functions.append(None)
...
class ContextManager:
    ...
    (Argument "ctx" in these functions)  上下文管理器
